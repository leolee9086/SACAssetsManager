export class DarkChannelDehaze {
    ...
    // 添加LUT计算方法
    calculateLUT(imageData, darkChannel, transmission) {
        const width = imageData.width;
        const height = imageData.height;
        const size = width * height;
        const lut = new Float32Array(size * 3); // RGB通道的LUT

        // 计算每个像素的LUT值
        for (let i = 0; i < size; i++) {
            const t = Math.max(transmission[i], 0.001);
            const scatteringFactors = [0.92, 0.96, 1.0];
            
            for (let c = 0; c < 3; c++) {
                const original = imageData.data[i * 4 + c] / 255;
                const A = this.params.atmosphere[c];
                ...
                lut[i * 3 + c] = Math.max(0, Math.min(1, result));
            }
        }
    ...
    applyLUT(imageData, lut) {
        ...
        const outputData = new Uint8ClampedArray(size * 4);
        
        for (let i = 0; i < size; i++) {
            for (let c = 0; c < 3; c++) {
                outputData[i * 4 + c] = lut[i * 3 + c] * 255;
            }
    ...
    process(inputImage) {
        return new Promise((resolve) => {
            ...
            // 计算LUT
            const lut = this.calculateLUT(imageData, darkChannel, refinedTransmission);
            
            // 应用LUT
            const outputData = this.applyLUT(imageData, lut);
