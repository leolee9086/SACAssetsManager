...
// 添加到文件顶部
const LMDB_CONFIG = {
    cacheSize: 1000 * 1024 * 1024, // 100MB cache
    maxDbs: 1,
    compression: true,
    // 清理配置
 ...
 * @param {string} [搜索关键词] 
 * @param {string[]} [扩展名列表] 
 * @returns {Promise<{结果: Array, 总数: number}>}
 */
export async function 查找子文件夹(目录路径, 搜索关键词, 扩展名列表) {
    return 安全数据库操作(async () => {
        const 开始时间 = Date.now();
        const 数据库 = await 获取数据库实例(目录路径);
        const 相对路径 = 转换为相对磁盘根目录路径(目录路径);

        // 手动收集结果到数组
        const 所有记录 = [];
        for await (const entry of 数据库.getRange({
            start: 相对路径,
            end: 相对路径 + '\uffff'
        })) {
            所有记录.push(entry.value);
        }

        // 过滤和转换结果
        const 根目录 = path.parse(目录路径).root;
        const 搜索词小写 = 搜索关键词?.toLowerCase();
        const 扩展名集合 = 扩展名列表?.length > 0 
            ? new Set(扩展名列表.map(ext => ext.toLowerCase()))
            : null;

        const 结果 = 所有记录
            .filter(value => {
                if (value.fullName === 相对路径) return false;
                
                if (搜索词小写 && !value.fullName.toLowerCase().includes(搜索词小写)) {
                    return false;
                }
                
                if (扩展名集合) {
                    const 扩展名 = path.extname(value.fullName).slice(1).toLowerCase();
                    if (!扩展名集合.has(扩展名)) return false;
                }
            ...
            .map(value => ({
                ...value.stat,
                path: 根目录 + value.stat.path
            }));

        console.log(`查询耗时: ${Date.now() - 开始时间}ms, 结果数量: ${结果.length}`);

        return {
            results: 结果,
            approximateCount: 所有记录.length
        };
    }, '查找子文件夹失败');
}
/**
