 ...
 */
export const toFile = async (sharpObj, options = {}) => {
    const buffer = await sharpObj.toBuffer(options);
    const format = options.format || 'jpeg';
    return new File([buffer], `image.${format}`, { type: `image/${format}` });
};
 ...
 * @param {Sharp} sharpObj - Sharp对象
 * @param {Object} [options] - 输出选项
 * @returns {Promise<string>} URL字符串
 */
export const toObjectURL = async (sharpObj, options = {}) => {
    const blob = await toBlob(sharpObj, options);
 ...
 * 转换为 HSV 数组
 * @param {Sharp} sharpObj - Sharp对象
 * @returns {Promise<Array<[number,number,number]>>} HSV数组
 */
export const toHSVArray = async (sharpObj) => {
    const rgbaArray = await toRGBAArray(sharpObj);
    return rgbaArray.map(([r, g, b]) => {
        r /= 255; g /= 255; b /= 255;
        const max = Math.max(r, g, b);
 ...
 * 转换为 CMYK 数组
 * @param {Sharp} sharpObj - Sharp对象
 * @returns {Promise<Array<[number,number,number,number]>>} CMYK数组
 */
export const toCMYKArray = async (sharpObj) => {
    const rgbaArray = await toRGBAArray(sharpObj);
    return rgbaArray.map(([r, g, b]) => {
 ...
 * 转换为 ImageBitmap
 * @param {Sharp} sharpObj - Sharp对象
 * @returns {Promise<ImageBitmap>} ImageBitmap对象
 */
export const toImageBitmap = async (sharpObj) => {
    const blob = await toBlob(sharpObj);
