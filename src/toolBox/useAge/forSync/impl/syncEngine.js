...
// 本地变更日志，用于断线重连时回放未同步的更改
const localChangesLog = new Map();
...
export function createSyncEngine(config) {
  ...
  // 为该房间创建变更日志记录
  if (!localChangesLog.has(cacheKey)) {
    localChangesLog.set(cacheKey, []);
  }
  ...
  const engine = {
    ...
    takeStateSnapshot: function() {
      ...
      try {
        ...
        // 复制refs
        if (store.store.state.refs) {
          for (const key in store.store.state.refs) {
            lastStateSnapshot.refs[key] = JSON.parse(JSON.stringify(store.store.state.refs[key]));
          }
        }
        
        // 复制states
        if (store.store.state.states) {
          for (const key in store.store.state.states) {
            lastStateSnapshot.states[key] = JSON.parse(JSON.stringify(store.store.state.states[key]));
          }
        }
      } catch (err) {
        console.error(`[SyncEngine] 创建状态快照出错:`, err);
      }
    },
     ...
     * @param {string} key - 键
     * @param {string|null} path - 属性路径
     * @param {any} value - 值
     * @param {Object} metadata - 元数据
     */
    recordLocalChange: function(type, key, path, value, metadata = null) {
      const changes = localChangesLog.get(cacheKey);
      if (!changes) return;
    ...
    replayLocalChanges: function() {
      if (!store || !this.isConnected() || !lastStateSnapshot) return;
      
      const changes = localChangesLog.get(cacheKey);
      if (!changes || changes.length === 0) return;
      
      if (config.debug) {
        console.log(`[SyncEngine] 准备重放本地更改，共${changes.length}条`);
      }
      ...
      setTimeout(() => {
        // 获取当前的远程状态
        const currentRefs = store.store.state.refs || {};
        const currentStates = store.store.state.states || {};
        
        // 比较并重放本地更改
        for (const change of changes) {
          try {
            const { type, key, path, value, timestamp } = change;
            const targetState = type === 'refs' ? currentRefs[key] : currentStates[key];
            ...
            console.error(`[SyncEngine] 重放本地更改出错:`, err);
          }
        }
        ...
        if (config.debug) {
          console.log(`[SyncEngine] 本地更改重放完成`);
        }
      }, 500);
    },
     ...
     * @param {string} key - 键
     * @param {any} value - 值
     * @param {Object} metadata - 元数据，包含来源信息等
     * @returns {boolean} 是否成功
     */
    setState: function(type, key, value, metadata = null) {
      if (!store || !store.store || !store.store.state) return false;
      ...
      try {
        ...
        // 获取当前值用于比较
        const currentValue = this.getState(type, key);
        ...
        // 记录本地更改以便断线重连时回放
        this.recordLocalChange(type, key, null, safeValue, metadata);
        ...
        if (store.sync) {
          setTimeout(() => {
            try {
              ...
              console.error(`[SyncEngine] 同步调用失败:`, err);
            }
          }, 0);
        }
    ...
    /**
     * 设置部分状态 - 支持增量更新
     * @param {string} type - 类型 (refs或states)
     * @param {string} key - 键
     * @param {string} propPath - 属性路径，可以是嵌套路径，如 'a.b.c'
     * @param {any} value - 值
     * @returns {boolean} 是否成功
     */
    setPartialState: function(type, key, propPath, value) {
      if (!store || !store.store || !store.store.state) return false;
      ...
      try {
        ...
        // 获取当前状态，如果不存在则创建
        let currentState = this.getState(type, key);
        if (!currentState) {
          // 创建一个空对象作为初始状态
          currentState = {};
          store.store.state[type][key] = currentState;
        }
        ...
        // 记录本地更改
        this.recordLocalChange(type, key, propPath, safeValue);
        ...
        // 特殊处理数组 - 如果目标是数组且要更新的值也是数组
        if (Array.isArray(target[lastPart]) && Array.isArray(safeValue)) {
          // 清空原数组但保留引用
          target[lastPart].length = 0;
          
          // 填充新内容
          for (let i = 0; i < safeValue.length; i++) {
            target[lastPart].push(safeValue[i]);
          }
        } else {
          // 其他情况直接赋值
          target[lastPart] = safeValue;
        }
        ...
        console.error(`[SyncEngine] 设置部分状态失败:`, err, {type, key, propPath});
        return false;
      }
    },
