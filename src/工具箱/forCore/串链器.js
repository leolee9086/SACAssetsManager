...
const 创建串链器 = () => {
  ...
  const 创建执行接口 = (全局态射表, 命名空间态射表, 初始值) => {
    ...
    const 代理 = new Proxy({}, {
      get(目标, 属性) {
        ...
        if (属性 === '分支') {
          return (条件映射, 默认分支) => {
            const 决策结果 = 扩展管理器.执行钩子('分支决策', 当前值, 条件映射);
            const 分支路径 = 决策结果.路径;
            
            // 执行对应分支
            let 分支结果;
            if (分支路径 !== '默认' && 条件映射[分支路径]) {
              分支结果 = 条件映射[分支路径](当前值);
              // 检查分支返回的结果是否是串链器对象，如果是则提取其值
              if (分支结果 && typeof 分支结果 === 'object' && 分支结果.值 !== undefined) {
                分支结果 = 分支结果.值;
              }
            } else if (默认分支) {
              分支结果 = 默认分支(当前值);
  ...
  const 主函数 = (初始值) => 创建执行接口(全局态射表, 命名空间态射表, 初始值);
  工具.标记串链器(主函数);
