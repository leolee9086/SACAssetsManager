      ...
      get(目标, 属性) {
        ...
        if (属性 === '分支') {
          return (条件映射, 默认分支) => {
            ...
            // 第二步：处理函数字符串条件（如果还没匹配）
            else if (!条件已匹配) {
              for (const [条件字符串, 处理函数] of Object.entries(条件映射)) {
                // 跳过默认分支和特殊条件
                if (条件字符串 === '默认' || 
                    条件字符串 === '空值' || 
                    条件字符串 === '未定义' ||
                    ...
                    条件字符串 === '零' ||
                    条件字符串 === '数组' ||
                    typeof 条件字符串 !== 'string') continue;
                
                // 处理函数字符串条件
                if (条件字符串.includes('=>')) {
                  try {
                    // 安全地将字符串转换为函数并执行
                    const 函数体 = `return (${条件字符串})(参数)`;
                    const 条件函数 = new Function('参数', 函数体);
            ...
            if (!条件已匹配) {
              const 预定义条件 = {
                '真值': Boolean(当前值),
                '假值': !当前值,
                '正数': typeof 当前值 === 'number' && 当前值 > 0,
                '负数': typeof 当前值 === 'number' && 当前值 < 0,
                '零': 当前值 === 0,
                '数组': Array.isArray(当前值)
              };
              
              for (const [条件名, 条件满足] of Object.entries(预定义条件)) {
                if (条件满足 && 条件名 in 条件映射) {
                  分支结果 = 条件映射[条件名](当前值);
                  条件已匹配 = true;
                  break;
