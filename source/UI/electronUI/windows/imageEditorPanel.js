...
export async function 打开图片编辑器窗口(imagePath) {
    ...
    const params = new URLSearchParams({
        ...
        modulePath:import.meta.resolve('./imageAdjuster/index.js')
    });
    newWindow.loadURL(import.meta.resolve('../templates/window.html') + '?' + params.toString());
...
export async function 打开图片画板窗口(imagePath) {
    ...
    const params = new URLSearchParams({
        ...
        imagePath: imagePath,
        baseStyleSrc: '/stage/build/app/' + baseStyleSrc,
        modulePath:import.meta.resolve('./draw/app.js')
    });
    newWindow.loadURL(import.meta.resolve('../templates/window.html') + '?' + params.toString());
    newWindowContents.openDevTools()
}
