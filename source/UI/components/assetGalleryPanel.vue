<template>
    <div @wheel="scaleListener" class=" fn__flex-column" style="max-height: 100%;" ref="root">
        ...
        <div class="fn__flex-column fn__flex-1" @dragstart.prevent.stop="onDragStart" style="width:100%" @mousedown.left="startSelection"
            @click="endSelection" @dblclick="openMenu" @mousedup="endSelection" @mousemove="updateSelection"
            @click.right.stop.prevent.capture="clearSelection"  @dragover.prevent>
            <!--选择框的容器-->
...
const { block_id, box } = appData.tab.data
const size = ref(100)
const root = ref('null')
...
const getSelectedItems = (event) => {
    ...
    let result = currentLayout.search({
        minX,
        minY,
        maxY,
        maxX,
    ...
    result.forEach(data => {
        ...
        data.selected = true
        //const element = root.value.querySelector(`[data-id="${data.data&&data.data.id}"]`)
        // element&&element.classList.add('asset-selected')
    });
};
/**
 * 拖放相关逻辑
...
const openMenu = (event) => {
    let assets = currentLayout.layout.filter(item => item.selected).map(item => item.data)
    plugin.eventBus.emit(plugin.events.资源界面项目右键, { event, assets }, { stack: true })
}
</script>