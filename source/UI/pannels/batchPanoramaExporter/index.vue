...
const generateWatermarkPreview = async (profileIndex) => {
  ...
  try {
    ...
    try {
      ...
      // 预处理水印选项 - 这是关键修改点，确保水印数据格式正确
      const watermarkOptions = {
        text: null,
        image: null
      };
      
      // 处理文字水印 - 按照PanoramaVideoGenerator.setTextWatermark的要求格式化
      if (profile.watermark.text.enabled) {
        // 计算字体大小
        let fontSize;
        switch (profile.watermark.text.fontSize) {
          case 'small': fontSize = Math.max(16, previewWidth / 60); break;
          case 'large': fontSize = Math.max(32, previewWidth / 30); break;
          case 'medium':
          default: fontSize = Math.max(24, previewWidth / 45); break;
        }
        
        // 只传递原始setTextWatermark方法期望的参数
        ...
        console.log('设置文字水印:', JSON.stringify(watermarkOptions.text));
      }
      
      // 处理图片水印 - 按照PanoramaVideoGenerator.setImageWatermark的要求格式化
      if (profile.watermark.image.enabled && profile.watermark.image.preview) {
        ...
        await generator.setImageWatermark({
          ...
          width: profile.watermark.image.size,
          opacity: profile.watermark.image.opacity
        });
        console.log('设置图片水印完成');
      }
      ...
      try {
        ...
        if (frameData && frameData.imageData) {
          ...
          // 手动添加水印
          if (profile.watermark.text.enabled) {
            addWatermarkToCanvas(canvas, profile);
          }
        }
      } catch (error) {
...
// 如果captureFrame不可用，手动添加水印到Canvas
const addWatermarkToCanvas = (canvas, profile) => {
  const ctx = canvas.getContext('2d');
  if (!ctx) return;
  ...
  let fontSizePixels;
  switch (fontSize) {
    case 'small': fontSizePixels = Math.max(16, canvas.width / 60); break;
    case 'large': fontSizePixels = Math.max(32, canvas.width / 30); break;
    case 'medium':
    default: fontSizePixels = Math.max(24, canvas.width / 45); break;
  }
  ...
  // 计算内边距
  const padding = Math.max(10, canvas.width / 100);
