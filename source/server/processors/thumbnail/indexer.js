...
export async function 写入缩略图缓存行(filePath, thumbnailPath, updateTime) {
    const { cachePath, root } = await getCachePath(filePath, 'thumbnailIndex.db')
    let 磁盘缩略图数据库 =  初始化数据库(cachePath,root)
    const stmt = 磁盘缩略图数据库.prepare('INSERT OR REPLACE INTO thumbnails (filePath, thumbnailPath, updateTime) VALUES (?, ?, ?)');
    const updateTimeValue = updateTime instanceof Date ? updateTime.getTime() : updateTime;
    stmt.run(filePath, thumbnailPath, updateTimeValue);
