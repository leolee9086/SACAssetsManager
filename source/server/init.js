...
import * as 日志数据库工具 from '../../src/toolBox/base/useEcma/forLogs/useLogDatabase.js'
import * as 日志处理器工具 from '../../src/toolBox/base/useEcma/forLogs/useLogProcessor.js'
...
// 初始化日志数据库
const 初始化日志数据库 = () => {
    return 日志数据库工具.初始化数据库({
        数据库名: 'SACAssetsManager_日志数据库',
        存储对象: '日志存储',
        版本: 1
    });
};
...
// 保存日志到数据库（支持批量保存）
const 保存日志到数据库 = (日志列表) => {
    return 日志数据库工具.批量添加日志条目(日志列表);
};

// 从数据库加载日志（支持分页）
const 从数据库加载日志 = (页码 = 0, 每页数量 = 100) => {
    return 日志数据库工具.查询日志条目({
        limit: 每页数量,
        offset: 页码 * 每页数量,
        orderBy: 'desc'
    });
