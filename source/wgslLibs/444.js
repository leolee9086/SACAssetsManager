export class DarkChannelDehaze {
    ...
    getTransmission(darkChannel) {
        const omega = 0.92;
        const minTransmission = 0.18;
        const maxTransmission = 0.96;
        
        // 添加边缘平滑处理
        const smoothedTransmission = darkChannel.map(dc => {
            // 使用更平滑的非线性调整
            const adaptiveOmega = omega * (1 + 0.1 * Math.pow(dc, 3)); // 降低非线性强度
            const rawTransmission = 1.0 - adaptiveOmega * dc;
            
            // 改进深度因子，使用更平滑的函数
            const depthFactor = Math.pow(dc, 0.85); // 提高指数使过渡更平滑
            const enhancedDepth = depthFactor * (1 + 0.08 * Math.pow(1 - depthFactor, 2));
            
            // 使用更平滑的混合权重
            const adjustedTransmission = 
                rawTransmission * 0.7 + 
                enhancedDepth * 0.2 + 
                Math.pow(1 - dc, 1.5) * 0.1;  
    ...
    process(inputImage) {
        return new Promise((resolve) => {
            ...
            for (let i = 0; i < width * height; i++) {
                const t = Math.max(transmission[i], 0.01);
                
                // 修改散射因子使其更均匀
                const scatteringFactors = [0.96, 0.97, 0.98];  
                ...
                for (let c = 0; c < 3; c++) {
                    ...
                    // 简化透射率调整，减少极端情况
                    const channelT = t * (
                        1.0 - (1.0 - scatteringFactors[c]) * Math.pow(1.0 - t, 0.75)
                    );
                    
                    const invT = 1.0 / Math.max(channelT, 0.01); // 提高最小值
                    
                    // 使用更温和的gamma值
                    const gamma = scatteringFactors[c] - 0.05;
