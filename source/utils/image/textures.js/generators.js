...
async function testTextureGenerator() {
    ...
    try {
        ...
        // 输出测试结果
        console.log('纹理生成测试结果:');
        for (const [type, typeResults] of Object.entries(results)) {
            console.log(`\n${type} 纹理测试:`);
            for (const [name, result] of Object.entries(typeResults)) {
                console.log(`- ${name}:`, {
...
// 添加一个辅助函数用于在页面上显示测试结果
async function displayTextureTests() {
    ...
    headerTitle.textContent = '纹理测试面板';
    ...
    texturesContainer.style.cssText = `
        display: flex;
    ...
    for (const [type, typeResults] of Object.entries(results)) {
        ...
        for (const [name, result] of Object.entries(typeResults)) {
            ...
            `;
            infoButton.onclick = () => {
                console.log(`${type} - ${name} 参数:`, result.params);
                alert(`参数信息已输出到控制台`);
            };
        ...
        texturesContainer.appendChild(typeGroup);
    }
    
    container.appendChild(texturesContainer);
...
displayTextureTests()
export { TextureGenerator, testTextureGenerator, displayTextureTests };
import './pattern.js'