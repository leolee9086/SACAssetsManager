...
fn main(@builtin(global_invocation_id) global_id: vec3u) {
    ...
    // 将坐标范围映射到 [0, 1] 范围，但避免到达边界
    let uv = vec2f(f32(global_id.x), f32(global_id.y)) / vec2f(f32(dims.x), f32(dims.y));
    
    // 添加偏移以打破中心对称性
    let offset = vec2f(0.37, 0.73); // 使用黄金比相关的值来避免明显的模式
    
    // 将缩放后的坐标限制在安全范围内
    let scale_factor = params.scale * 0.95;
    let p = vec3f(
        (uv.x + offset.x) * scale_factor,
        (uv.y + offset.y) * scale_factor,
        params.time
    );
    ...
    // 改进的亮度和对比度调整
    let mid = vec3f(0.5);
    let contrast_adjusted = mix(mid, color, params.contrast);
    let adjusted_color = clamp(
        contrast_adjusted + (params.brightness * 0.5), // 将亮度调整范围减半
        vec3f(0.0),
        vec3f(0.98) // 限制最大亮度
    );
