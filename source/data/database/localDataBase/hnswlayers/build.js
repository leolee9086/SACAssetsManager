...
function 链接邻居(数据集, 数据项, 邻居列表, 模型名称, hnsw图层级, hnsw层级映射) {
    ...
    层级邻接表.items = 邻居列表.map(邻居记录 => { if (邻居记录.data && 邻居列表.distance) { return { id: 邻居记录.data.id, distance: 邻居列表.distance } } });
    let 最近邻居 = null
    let 最近邻居距离 = Infinity
    // 反过来,连接邻居项与当前项
    for (let 邻居节点记录 of 邻居列表) {
        ...
        //这实际上是一个最大堆
        let 候选邻居堆 = new 最小堆((a, b) => b?b.distance:1 - a?a.distance:1);
        if (!候选邻居的邻接表) {
            console.log(邻居节点记录, hnsw图层级)
            重建数据集的层级映射(数据集, hnsw层级映射, 邻居节点记录.data.id)
            continue
        }
        候选邻居的邻接表.items.forEach(item => item&&候选邻居堆.push(item));
        try{
        数据项 &&邻居节点记录&& 候选邻居堆.push({ id: 数据项.id, distance: 邻居节点记录?邻居节点记录.distance:1 });
        }catch(e){
...
export async function 为数据项构建hnsw索引(数据集, 待插入数据项, 模型名称, hnsw层级映射) {
    ...
    if (!当前层入口点) {
        setTimeout(() => 为数据项构建hnsw索引(数据集, 待插入数据项, 模型名称, hnsw层级映射), 100)
    }
    let 当前距离 = 2
