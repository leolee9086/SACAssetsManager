        ...
        if (data && !访问过的节点.has(data.id)) {
            ...
            if (!邻居节点的当前层级邻接表) {
                ...
                if (重建尝试次数 < 最大重建次数) {
                    ...
                    // 如果重建后仍然无法获取邻接表，跳过此节点
                    if (!邻居节点的当前层级邻接表) {
                        console.warn(`邻居节点${邻居节点.id}的${当前层级}层邻接表重建后仍不存在，跳过此节点`);
                        continue;
                    }
                } else {
                    // 如果已达最大重建次数，跳过此节点
                    console.warn(`邻居节点${邻居节点.id}的${当前层级}层邻接表不存在且已达最大重建次数，跳过此节点`);
                    continue;
                }
            }
    ...
    for (let 邻居节点记录 of 邻居列表) {
        ...
        if (!候选邻居的邻接表) {
            ...
            if (重建尝试次数 < 最大重建次数) {
                ...
                // 如果重建后仍然无法获取邻接表，跳过此节点
                if (!候选邻居的邻接表) {
                    console.warn(`邻居节点${邻居节点记录.data.id}的${hnsw图层级}层邻接表重建后仍不存在，跳过此节点`);
                    continue;
                }
            } else {
                // 如果已达最大重建次数，跳过此节点
                console.warn(`邻居节点${邻居节点记录.data.id}的${hnsw图层级}层邻接表不存在且已达最大重建次数，跳过此节点`);
                continue;
            }
        }
...
function 贪心搜索当前层级构建入口(数据集, 当前层入口点, 待插入数据项, hnsw索引名称, 当前层级, 模型名称, hnsw层级映射) {
    ...
    while (改进 && 搜索次数 < 最大搜索次数) {
        ...
        if (!当前层邻接表 && 重建尝试次数 < 最大重建次数) {
            ...
            // 如果重建后仍然无法获取邻接表，使用初始入口点并跳出循环
            if (!当前层邻接表) {
                console.warn(`数据项${最近点.id}的${当前层级}层邻接表重建后仍不存在，使用初始入口点`);
                最近点 = 初始入口点;
                最近距离 = 计算数据项距离(初始入口点, 待插入数据项, 模型名称);
                break;
            }
        }
        // 如果邻接表不存在且已达到最大重建次数，使用初始入口点并跳出循环
        else if (!当前层邻接表) {
            console.warn(`数据项${最近点.id}的${当前层级}层邻接表不存在且已达最大重建次数，使用初始入口点`);
            最近点 = 初始入口点;
            最近距离 = 计算数据项距离(初始入口点, 待插入数据项, 模型名称);
            break;
        }
